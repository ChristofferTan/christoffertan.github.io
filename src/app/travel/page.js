...
function Section({ id, title, children, className = "", compact = false }) {
    ...
    return (
      <section id={id} className={`mx-auto max-w-[1180px] px-5 sm:px-6 md:px-8 ${pad} ${className}`}>
        {title && (
      ...
      <nav className="mx-auto max-w-[1180px] px-5 sm:px-6 md:px-8 h-14 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          <ul className="flex gap-4 text-sm text-zinc-300">
            {NAV.map(({ label, href }) => (
              <li key={label}>
                <a href={href} className="relative hover:text-white transition-colors">{label}</a>
              </li>
            ))}
          </ul>
  ...
  return (
    <main className="min-h-screen bg-[#0c0d10] text-zinc-100">
      ...
      <Section compact className="pb-2">
        ...
        Traveling has always been my way of collecting stories—every dot here marks a city where I’ve found new perspectives, flavors, and friendships. Hover to uncover the places behind the memories.{" "}
        <span className="text-purple-400">Purple</span> highlights capitals I’ve visited, and{" "}
        <span className="text-blue-400">Blue</span> shows other cities I’ve explored.
        </p>
        </Section>
        ...
        <Section id="map" compact className="pt-2">
            ...
            content={
                ...
                <>
                    {tip.data?.photo && <img src={tip.data.photo} alt="preview" loading="lazy" />}
        ...
        <Section id="list">
        {(() => {
            // Build a country -> [cities] map
            const byCountry = VISITED_CITIES.reduce((acc, cur) => {
                ...
                {countryNames.map((c) => {
                    ...
                    <li key={c} className="h-full rounded-xl border border-white/10 bg-white/5">
                        ...
                        <summary className="flex cursor-pointer items-center justify-between gap-3 px-3 py-2
                            ...
                            <span className="shrink-0 grid place-items-center h-6 w-6 rounded-md border border-white/10 bg-white/5
                                            text-white/60 group-open:rotate-180 transition-transform">
                            ▾
                            </span>
                        </summary>
                        <div className="px-3 pb-2 pt-1">
                            ...
                            {cities.map((m) => (
                                ...
                                className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5
                                            px-2.5 py-1.5 text-sm text-white/85 hover:bg-white/[0.08] transition-colors"
                                >
                                <span
        ...
        <footer className="border-t border-white/10 bg-[#0c0d10]/70 backdrop-blur supports-[backdrop-filter]:bg-[#0c0d10]/70">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <a href={`mailto:${ME.email}`} className="hover:text-cyan-300 transition-colors flex items-center gap-1">
                    <Mail size={16} /> <span>Email</span>
                </a>
                <a href={ME.linkedin} target="_blank" rel="noreferrer" className="hover:text-cyan-300 transition-colors flex items-center gap-1">
                    <Linkedin size={16} /> <span>LinkedIn</span>
                </a>
                <a href={ME.github} target="_blank" rel="noreferrer" className="hover:text-cyan-300 transition-colors flex items-center gap-1">
